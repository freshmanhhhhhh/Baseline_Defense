# classify prompt using Prompt-Guard-86M
# nohup python judgebyPG.py --ratio balance --harm_dset jade > ./logs/PromptGuard-86M/combine_jade_benchmark_easy_1k1k.log 2>&1 & 
...
for p in prompts:
    ...
    # a list of dict with label and score as keys
    output = classifier(p)
    ...
    # 解析响应    
    toxicity_classification = label2type[output[0]['label']]

    # 构建结果记录
    result = {
        "prompt": p,
        "toxicity_classification": toxicity_classification,
        "toxicity_type": output[0]['label'],
...
print("[*]PERFORMANCE[*]\n\
        ...
        recall: {}\n\
        f1: {}\n\
        fpr: {}".format(acc, precision, recall, f1, fpr))
